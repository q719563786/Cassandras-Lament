...
- 新增 ：零依赖的覆盖率基线工具（用标准库 trace，
  不引入 coverage 包）。基线（269 项测试）：**总体 80.4%**，
  可执行行 6948 / 已执行 5589；低于 80% 的模块 8 个。
  覆盖最低的五个： 48.0%、 64.5%、
   66.2%、 68.5%、 71.1%。
...
- 诚实说明：`judgments` 实测 281 MB（占全库约 31%、占每日增量约一半）受上述
  保护无法清理，因此本改动把日增从约 26 MB 压到约 14 MB 并收敛约 41% 的体积，
...
- 移除 `/api/dashboard` 接口。它界面从未调用、无测试覆盖，却会把全部预测正文
  读出来再在 Python 里筛选，实测返回 **17.5 MB**；功能与界面在用的
  `/api/risk-dashboard`（141 KB）重叠，因此整体删除，含随之失效的
  `SignalsService.high_alerts()`。
- 修正两处文档与实现不符：`retention.py` 声称有"趋势降采样"、设置页一处说明
  也照抄了这句话，但代码里并无实现，均已改正。
