...
#
# 远程研判量由四道闸共同决定，任何一道单独放大都不会真的提高吞吐：
#   ① 频率闸 `_remote_due()`：距上次远程完成多久才开新一轮（low 夜间窗口 /
#      medium 6 小时 / high 见下）
#   ② 每轮名额 `REMOTE_SLOTS_PER_ROUND`：开闸那一轮最多送多少个作业
#   ③ 瞬时速率闸 `remote_ai.REMOTE_MIN_INTERVAL_SECONDS`：相邻两次远程请求的
#      最小间隔（基线 5 秒，**对端回 429 时自适应成倍放慢**，上限 300 秒）。
#      **前三道管"一轮/一天送多少"，只有这一道管"每秒发多快"** —— 少了它，25 个
#      槽位会在几秒内被背靠背发完。
#   ④ 每日上限 `ai_settings.daily_budget`（默认 2000，可在设置页改）
#
# ── 2026-09-23 修订：② 从 25 降到 6 ─────────────────────────────────────
#
# 旧值与 ③ 的关系是"25 × 5 秒 = 125 秒 ≤ 300 秒轮周期"，前提是 ③ 恒为 5 秒。
# 实测之后这个前提不成立了：Agnes 免费档的实际放行速率约 **1 次/分钟**（20 秒
# 间隔发 6 次只过 2 次），③ 会被自适应拉到 60~300 秒。届时 25 个槽位意味着
# **一轮最长 125 分钟**，而调度器是单线程、任务串行 —— 认知轮会把采集、态势、
# 趋势一起饿死（`radar_scheduler` 里"冷启动补抓饿死态势块"就是同类故障）。
#
# 取 6 的依据：6 × 5 秒 = 30 秒（顺境，远放得下）；③ 退到 60 秒时 6 × 60 = 360
# 秒，略超轮周期，但 `run_due` 还会按 `remote_ai.REMOTE_ROUND_TIME_BUDGET_SECONDS`
# （120 秒）现算实际条数并取小 —— 两道保险不重复，一道管"配额"、一道管"本轮
# 真的花多久"。顺带一个好处：一轮只放 6 条，撞上对端限流时最多烧掉 6 个请求
# 就会停发（429 立即中止本轮），而不是 25 个。
#
# **不要再把 ② 调回两位数**：真要吞吐就换 provider 或调高 `daily_budget`，
# 靠加大 ② 只会把"轮次被拖长"和"限流被烧穿"两件事一起放大。
REMOTE_INTERVAL_HOURS_HIGH = 0.25
REMOTE_INTERVAL_HOURS_MEDIUM = 6
REMOTE_SLOTS_PER_ROUND = 6
...
class CognitionController:
    ...
    def risk_dashboard(self, source_states=None, limit=3):
        ...
        # v1.2 第三波：**读路径不再写库**。
        # 这里原本调 impacts.auto_confirm_all()，等于「打开一次首页就改动不可变预测
        # 账本」——刷新页面改变账本状态，且「当时用户看到的是什么」无法复现，
        # 审计与回溯失去基准。已删除。
        #
        # 现在写账本只发生在两条路径上：
        #   1. 周期认知任务（cognition.process_once → _auto_confirm_candidates），
        #      按用户设定的策略自动确认 **L3**（L4 与 E1 一律留给用户手动确认）；
        #   2. 用户在界面上的显式确认。
        # 本函数只读：需要的「待确认候选」照常由下方查询取出。
