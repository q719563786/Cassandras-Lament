...
#      最小间隔（5 秒 = 12 次/分钟）。**前三道管"一轮/一天送多少"，只有这一道管
#      "每秒发多快"** —— 少了它，25 个槽位会在几秒内被背靠背发完，直接撞 Agnes
#      的 20 RPM。
#   ④ 每日上限 `ai_settings.daily_budget`（默认 2000，可在设置页改）
#
# 四者的匹配关系：应用每 5 分钟轮询一次，high 档间隔 15 分钟 ≈ 每 3 轮开一次闸，
...
# 每轮 25 个受 ③ 约束只用掉 125 秒（25 × 5 ≤ 300 秒轮周期），放得下；
# 改大 ② 之前先重算这道不等式。
#
# **不要为了逼近 20 RPM 改成并发请求**：Agnes 免费版限的是每分钟 20 次。
# 原先这里写的是"远见是顺序请求，结构上撞不到 RPM"——**该判断已被真库数据推翻**
# （2026-09-16 安装后 15 小时：succeeded 152 / rate_limit 23，13% 被限流；
# 那 100 个作业是在 6 秒内排进队列、由 `run_due` 一轮里连发出去的）。
# 顺序请求只有在"单次调用很慢"时才会自然拉开间隔；Agnes 这类秒级模型不会。
# 现在这道闸固定在传输层（`remote_ai._remote_pacer`），线程池并发也一样受限。
