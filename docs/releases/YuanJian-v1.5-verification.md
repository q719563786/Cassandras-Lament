...
**分支 / 提交**：`mystique-theme` @ `4b28fc3`（45 files changed, +4001 / −160）
...
| 模块 | 项数 | 覆盖 |
|------|------|------|
| `tests/test_situation_layers.py` | 17 | GeoJSON 解析（USGS/EONET/GDACS 夹具、跳脏数据）＋幂等 upsert＋只读接口契约＋参数越界 400＋令牌强制 |
...
| 层 | 脚本 | 结果 |
|----|------|------|
| 内容级（解 exe 里的 PYZ + 读打包静态资源） | `build-artifacts/lead_verify_installed_v15.py` | 32 个 Python 标识符全在（含 4 个批次）＋ 14 项静态资源断言全 OK；主题 CSS 与底图 GeoJSON **逐字节一致** |
| 启动烟测（无窗口真跑，只读接口） | `build-artifacts/lead_smoke_v15.py` | 19 项全过；版本自报 1.5.0；态势 `/layers`、`/points`、`/geo/ne_110m_land.geojson`（200 / `application/geo+json` / 127 多边形）；5 个越界参数全 400；跑完 `POST /api/shutdown` 正常退出 |
| 行为级（临时库 + 从库里读回 + 变异对照） | `build-artifacts/lead_verify_v15_e2e.py` | 36 项全过（见 2.2） |

### 2.4 发布闸门

```
tools/privacy_scan.py --committed   →   committed_files=149  safe=True  blocked=0  findings=0  (exit 0)
```
...
| 项 | 值 |
|----|-----|
| 产物 | `dist/YuanJian/YuanJian.exe`（onedir），6,940,628 B |
| exe sha256 | `2754BBFD4C493D93E3EF7E64CEAE7EEB9C02C342C4E674574C2C8E9CB1E57EC0` |
| 主题 CSS | `17DA4778…A44943`（53519 B，交付件逐字节一致） |
| 底图 GeoJSON | `9E0729EE…2835D9`（138160 B，交付件逐字节一致） |
| 安装位置 | `D:\远见\程序` |
| 回滚点 | `D:\远见\程序-旧-20260921-145135`（旧版 `CFDB9ECF…`，保留不删） |
| `data-dir.txt` | `D:/远见/数据`（升级后保留） |
| 首次启动耗时 | **5.21 s**（进程拉起 → 版本接口可用；含 v8 建表） |
...
- `situation_events` 现为 0 行：态势源由调度器按期抓取，装后首次启动尚未到抓取点 —— 属预期。
...
python -m unittest discover -s tests          # 723 项 × 3 轮；噪声用例单独重跑见下
