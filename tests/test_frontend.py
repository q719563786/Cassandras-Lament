...
"""
    ...
    def test_views_use_only_local_module_contracts(self):
        ...
        # v1.0 行动雷达：app.js 用 renderView() 刷新视图，
        # 不再直接调用 /api/cognition/run。/api/shutdown 仍保留。
    ...
    def test_today_view_consumes_backend_gyw_and_falls_back_to_ui_template(self):
        """v1.0 行动雷达首页：直接消费 /api/risk-dashboard 返回的 items，
        按 mode=action/watch + alert_level=L4/L3 渲染行动卡，并显示今日低后悔动作。
        旧的 gyw 深度预知卡片已被移除（按 P0 用户要求："首页不该出现噪音"）。"""
        view = (STATIC / "js" / "views" / "today.js").read_text(encoding="utf-8")
        # 风险面板接口合约：dashboard.items 每项含 mode/alert_level。
        self.assertIn("/api/risk-dashboard", view)
        self.assertIn("dashboard?.items", view)
        # 风险等级与模式筛选：L4 立即行动 / L3 准备观察。
        self.assertIn("alert_level === 'L4'", view)
        self.assertIn("alert_level === 'L3'", view)
        # 今日低后悔动作固定区域（4 条）。
        self.assertIn("今日低后悔动作", view)
        self.assertIn("safe-list", view)
        # 预测进度 KPI 与过期预测提醒（P3）。
        self.assertIn("/api/forecasts/progress", view)
        self.assertIn("overdue", view)
        # 告诉远见入口仍保留。
        self.assertIn("tellBoxHtml", view)
