...
def _valid_candidate(*, vague=False):
    ...
    return {
        ...
        "resolution_criteria": (
            "判定依据：以官方公布的报销比例下调文件为准，"
            "对照可观测事实核验；该信号在窗口开始后首次被观测到才算发生。"
        ...
        "probability_low": 0.55,
        "probability_high": 0.78,
        # v1.4（R-08）：可观测信号必须事件级特化 —— 至少一条要含本事件特有的实体，
        # 或与事件标题共享一段 ≥4 字的连续片段。夹具原先写的是类别模板短语
        # （"主管部门公布正式文件"），新闸门会正确地把它拦在账本之外。
        "observable_signals": "医保政策调整的报销比例下调文件公布；定点医院结算系统更新",
        "causal_chain": "政策变化 -> 自付成本变化",
        "supporting_evidence": "S-1",
        "opposing_evidence": "执行细则待公布",
